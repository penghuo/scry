...
**Reviewer:** Momus v3.1 (full-document pass, DataSketches research incorporated)
**Rating:** ⭐⭐⭐ (OKAY — Workable with Risk)
**Previous:** v1 ⭐⭐⭐, v2 ⭐⭐⭐⭐ (DataSketches-only refresh), v3 ⭐⭐⭐ (full-document pass)
...
The RFC articulates a genuine gap — investigation as a first-class system object — with strong external evidence. But it is 60% problem statement and 40% solution sketch. Solution sections name constructs (signal catalog, 5 primitives, evidence states) without specifying schemas, APIs, PPL syntax, state transitions, or index mappings. An implementer would know *what* to build but not *how*. Two cited statistics are unverifiable. The companion DataSketches design proves the RFC's most ambitious subsystem is feasible — with 3 concrete evidence points: (a) OpenSearch's existing `cardinality` agg already uses HLL++ internally, proving the shard-then-reduce sketch pattern works, (b) Druid's dual Build/Merge pattern provides a proven reference architecture adopted by 5 production systems, and (c) clean extension points are identified (Data Prepper `AggregateAction` + OpenSearch `SearchPlugin.getAggregations()` SPI). This companion design needs to be incorporated into the RFC to make it self-contained.
...
- **"Investigation summaries" groups two items at different maturity levels, but both follow the same lifecycle.** "Derived metrics/histograms" already exist in Data Prepper (ref09 documents the `aggregate` processor with `histogram` action). "Mergeable sketches" require new infrastructure — but the companion DataSketches design (`oracle-datasketches-design.md`) provides concrete implementation: a `sketch_aggregate` AggregateAction with Java code (§2.1), pipeline YAML config (§2.2), and a sketch-per-signal-type table (§2.3). Both derived metrics and sketches follow the same BUILD→MERGE→ESTIMATE lifecycle pattern used by Druid, Spark, and 3 other production systems (`librarian-datasketches-integrations.md` §7). The RFC itself lacks this detail — it should incorporate or reference the companion design.
...
- **No Data Prepper pipeline config shown in the RFC.** The companion DataSketches design (`oracle-datasketches-design.md` §2.2) provides a complete pipeline YAML with `sketch_aggregate` processor config, identification keys, group duration, and per-sketch-type parameters. This fills the gap — but the RFC itself should incorporate or reference it to be self-contained.
...
- **No PPL examples in the RFC, but the companion design validates the primitive model.** The RFC says "PPL, DSL, or API can all carry these primitives" but shows zero examples. The companion DataSketches design (`oracle-datasketches-design.md` §5) maps 4 of 5 primitives to concrete PPL functions: `sketch_count_distinct`→discover, `sketch_percentile`→narrow, `sketch_diff`→pivot, `sketch_intersect`→compare. The sketch→primitive mapping (`librarian-datasketches-observability.md` "Sketch → Investigation Primitive Mapping") confirms the RFC's investigation primitive design is sound — each sketch family maps 1:1 to a cognitive investigation operation. The RFC should incorporate these PPL examples to be self-contained. `continue` (stateful investigation resumption) remains unspecified anywhere.
...
**Issues:**
- **Ignores existing MCP tool surface.** Ref12 shows OpenSearch already exposes 30+ tools via MCP server at `/_plugins/_ml/mcp`, including SearchAlertsTool, SearchAnomalyDetectorsTool, SearchAnomalyResultsTool, PPLTool. The RFC should specify how investigation objects extend this existing surface — new tool registrations? New tool types? The companion DataSketches design (`oracle-datasketches-design.md` §6) defines 4 concrete MCP tools with input/output schemas: `sketch_discover` (HLL→estimate+bounds+RSE), `sketch_narrow` (KLL→dimension-grouped percentiles), `sketch_pivot` (Theta AnotB→new/removed counts+jaccard), and `sketch_compare` (Theta intersection→overlap+exclusive counts+jaccard). All accept standard OpenSearch query filters and ISO time ranges, and all include error bounds in output. The RFC itself has none of this — it should incorporate or reference the companion design.
...
**Issues:**
- **P0 scope is very large.** Signal catalog + investigation summaries + 5 query primitives + investigation context + evidence/conclusion state + governance boundaries — this is essentially the entire RFC minus agent APIs. The RFC itself provides no timeline or effort estimate, but the companion DataSketches design (`oracle-datasketches-design.md` §8-9) provides concrete estimates for the sketches subsystem: Phase 1 (HLL+KLL, discover+narrow) = 10-14 weeks, Phase 1+2 (+Theta, +pivot+compare) = 16-22 weeks, all phases = 24-30 weeks. These estimates are broken down by component (Data Prepper AggregateAction 2-3w, OpenSearch sketch_build agg 3-4w, PPL functions 2-3w per phase, MCP wrappers 1w, integration tests 2-3w). The RFC should incorporate these estimates and produce equivalent ones for signal catalog, investigation state, and governance.
...
3. **PPL examples** for each of the 5 query primitives — *companion design addresses 4 of 5 primitives (`oracle-datasketches-design.md` §5.2); needs incorporation into RFC*
...
10. **End-to-end walkthrough** — one scenario traced through all 5 layers with concrete queries — *companion design provides a 4-step p99 spike walkthrough (`oracle-datasketches-design.md` §7); needs incorporation into RFC*
11. **Error bounds** for sketch-based primitives — *companion design and research quantify these: HLL ±1.3% RSE at lgK=14, KLL ±1.33% rank error at K=200 (`librarian-datasketches-observability.md`); needs incorporation into RFC*
...
11. **Incorporate the companion DataSketches design into the RFC.** The companion design (`oracle-datasketches-design.md`) provides index mappings, PPL functions, MCP tools, pipeline config, a walkthrough, and phased timeline estimates that the RFC currently lacks. Either inline the key artifacts or formally reference the companion design so the RFC is self-contained. This is the single highest-leverage improvement — it transforms §6.1, §6.3, §6.4, and §8 from concept-only to concrete.

### Nice to Have

12. **Add anti-principles** to Section 5 (what the RFC explicitly does not optimize for).
13. **Add missing FAQs** (Application Analytics, Notebooks, performance overhead, migration, error bounds).
...
| **Context Completeness** | Pass | Strong external references. Companion DataSketches design provides full architectural context: extension points (`librarian-opensearch-architecture.md`), cross-system precedent (`librarian-datasketches-integrations.md`), sketch→primitive mapping (`librarian-datasketches-observability.md`), and concrete implementation (`oracle-datasketches-design.md`). Internal OpenSearch capability inventory still incomplete in the RFC itself. |
| **Big Picture** | Pass | Clear purpose, logical flow, honest about Phase 1 limitations, good problem-to-solution traceability. |

**Overall: ⭐⭐⭐ — The RFC correctly identifies a real gap and proposes the right structural response. The companion DataSketches research and design prove the most ambitious subsystem is feasible and well-scoped. But the RFC itself stops at naming constructs without specifying them — the companion design's detail hasn't been incorporated. An implementer would need to make dozens of unguided design decisions for non-sketch subsystems (signal catalog, investigation state, governance). The path to ⭐⭐⭐⭐ is clear: incorporate the companion design and produce equivalent detail for the remaining subsystems.**
