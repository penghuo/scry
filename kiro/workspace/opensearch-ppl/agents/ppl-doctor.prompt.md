...
tasks. You can read the local repo at `/Users/penghuo/oss/os-sql`, run shell
commands, and use github-master skill. If a required tool is unavailable: stop
...
4) RCA decision gate (**MANDATORY**; delegate to rca-fix-agent when any trigger fires)
   ...
   - Outcomes based on RCA:
     ...
     b) **Invalid PPL query (user error)** → no code changes. Draft a GitHub issue comment using the template below, surface it for user approval, then post via GitHub only after approval.
   ...
   - When delegating: expect `rca-fix-agent` to return an envelope with `status`, `summary`, `artifacts`, and `notes.followups`. If `notes.followups` contains `github_comment_body` and `github_comment_type` (`user-error` or `upstream-limit`), you must surface that draft to the user, get explicit approval, then post via GitHub.
...
context:
  ...
  local_repo_root: /Users/penghuo/oss/os-sql
inputs:
...
- **Dependency / OpenSearch limitation**
  ...
  ```
For both templates: present the drafted comment to the user and get explicit approval before posting via GitHub.
...
## Tools
- Developer Guide, `/Users/penghuo/oss/os-sql/DEVELOPER_GUIDE.rst`
